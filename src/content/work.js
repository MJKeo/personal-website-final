...
export const FLAGSHIPS = [
  ...
  },
  {
    ...
    // ---- Detail-page fields (rendered by the WorkDetail screen via bySlug) ----
    tagline: 'June 2022 - May 2025',
    writeup: [
      "I spent 3 years as a software engineer at Meta, working on Instagram in San Francisco and shipping creator-facing features to millions of users. The job was a constant balancing act: build quickly inside enormous, largely undocumented systems without compromising on quality. A mistake there doesn't hide, it lands in front of real people at a scale where even the rare edge cases happen thousands of times a day. And as the company got leaner the role kept widening. We did whatever was needed to get the product out the door, which over time grew me from an iOS developer to a full-stack developer, a data scientist, a product manager, and a mentor. That environment built four skills I lean on constantly: shipping fast without breaking things, wearing whatever hat the product needs, treating data as king, and managing scope well beyond my own code.",

      { heading: 'Working fast without breaking things' },
      "The baseline expectation at Meta is a demanding pair: execute quickly on parallel projects in fragile systems with absurdly high visibility. One example of this was expanding Creator Achievements. We had inherited a system from another team and my job was to extend it beyond reels to our other media types. It sounds simple on paper, but each action on each media type is handled in a unique way in a different part of the codebase and owned by completely different teams. Your job then is to identify all of these different flows, locate where they're triggered in the codebase, reverse engineer how they work or spin up a conversation with their POCs, handle any unexpected speed bumps encountered (in this case, handling all the deprecated media types that still exist as previously published content), and then plan the proper changes to make. Writing the actual code is the easy part, it's the consistent ramping up on new spaces to the degree that you can safely make the correct changes at a fast enough speed to not cause timeline delays that serves as the real challenge.",
      "The other half of shipping safely is polish, and this is one of my biggest strengths: finding the edges of a system. Every feature has far more scenarios than the base flow that gets designed out. I hunt those down during planning rather than discovering them in production, make decisions on how to handle them gracefully and intuitively, and then back it with extensive testing in the final stretch. When your product goes out to tens of millions of people you can be sure that whatever you don't catch will get experienced by a good number of users. That being said, thorough and slow are not the same thing: most of it is asking the right questions early, when accounting for them is still cheap.",

      { heading: 'Wearing whatever hat the product needs' },
      "Titles at Meta describe where you start, not what you do. As the org got leaner there weren't enough specialists to go around, so when something needed doing and the people who would normally do it were stretched thin, you stepped up and learned it. Creator Goals was the clearest version of this for me. It was a brand-new feature in a tech stack I had never touched, with designs that weren't finalized until we were two thirds of the way through execution, and limited data science support. While I had my own features to lead and develop it was common to jump around and assist other features get out on time, whether that be on client-facing or backend development, or even designing how we'll capture and interpret user data. Projects are chaotic by nature, our philosophy was everyone lends a hand where it's needed, and if it's not something they already know then it's a great time to learn.",

      { heading: 'The value of data' },
      "Everything at Instagram ships through A/B experiments, and engineers don't just implement the feature, we own the readout: what moved, what didn't, and a theory for why, backed by supporting metrics. It's a discipline I came to love, and it's where I did the work I'm most proud of.",
      "The push came from a failure. A key engagement metric dropped 34% and went unnoticed for roughly 3 weeks. The direct cause was a silent failure in one of our features, but the alarming part was why it took so long to catch: like many teams sprinting on product work, we had treated logging as an afterthought, and the foundation showed it. Our logs retained only 2 days of data, queries failed outright on any window longer than 3 hours, iOS and Android logged the same events in incompatible formats, and key fields were simply missing. We were operating in a black box, which meant decisions on what to build and whether a feature had a positive impact on our users were being made with far less information than they deserved.",
      "I took it upon myslf to redesign our entire data collection system from the ground up, aimed at maximizing ease of insertion into features and quality of data readouts. I treated it like a product in its own right: its users were the engineers who had to implement the logging and the data scientists who had to query it, so I interviewed both about their pain points and studied how other teams at the company handled theirs. The rebuild restructured events for fast queries and precise monitoring, introduced shared data objects so every platform logged the same thing the same way, replaced free-form fields with strongly typed, server-defined schemas (which also closed gaps that risked capturing sensitive data), and root-caused the query failures to a single overloaded field that was exhausting memory. This wasn't part of my scope for the half, it was a conviction I pursued on top of my expected production.",
      "It was a resounding success. I added it to our new product, Goals, ready from day one of experimentation. Instead of retrofitting analytics after the fact we caught bugs almost immediately, watched completion and retention in real time, and used those insights to shape the next 2 months of product iteration. When the product itself didn't ship, that analysis became the team's primary deliverable, the difference between a failed experiment and a genuinely valuable learning process. Data engineers began pointing other teams to the system as the standard for data collection, and I authored an org-wide writeup of the learnings so they wouldn't stay ours. From that point forward my team treated logging as a core attribute of any feature developement.",

      { heading: 'Owning more than my own execution' },
      "As I progressed from a new grad to a full engineer, my scope grew from just worrying about getting my own work done to coordinating the efforts of multiple engineers. For the data overhaul I oversaw the efforts to add the new system to each independent feature of our Goals product and every subsequent project. When we were iterating on a bundle of interdependent experiments in the Achievements space, communication had begun to deteriorate and timelines silently began to slip. Although I wasn't meant to lead this project, I took it upon myself to right the ship: aligning on what needed to get done, by whom, by when, and what else had to happen for the whole thing to run smoothly. In an environment where one mistake would catastrophically pollute results, we were able to get back on track and ship successfully.",
      "Day to day, that kind of ownership is a series of small judgment calls: getting the right people into a room for 30 minutes instead of letting a decision drag across days of half-engaged async threads, sending design one organized doc of design concerns instead of a stream of messages, establishing early what gets cut when timelines demand it, and running experiment readouts with leadership.",
      "Although hiring restrictions meant I was always most junior member of my team I grew into a mentorship role for some of my peers, even leading to one of their promotions.",

      { heading: 'What it taught me' },
      "Meta gave me the discipline of scale. Speed and quality stop being a tradeoff once you're deliberate about where the risk actually lives: most of protecting quality happens in the questions you ask during planning and the rigor of your testing, not in slowing down the middle. Data isn't a report you write after the fact, it's the steering wheel, and instrumentation deserves to be designed like a product because your own team are its users. Ownership is about the outcome, not the assignment, and the features I'm proudest of worked because someone was willing to do the unglamorous connective work between the commits.",
      "More than anything, those 3 years taught me how to walk into a system I've never seen, in a stack I may not know, and become useful quickly. Every feature was a new corner of the codebase and a new set of things to learn, so ramping up fast stopped being an event and became the routine. That's the muscle I've leaned on in everything I've built since.",
    ],
  },
  {
...
export const EXPERIENCES = [
  {
    slug: 'independent-ai-product-development',
    category: 'experience',
    href: '/experience/independent-ai-product-development',
    title: 'Independent AI Product Development',
    cover: '/images/experiences/independent-ai-product-development/banner.png',
    summary:
      'After three years at Meta I left to spend a year building AI-native products end to end on my own, a deliberate progression from focused LLM experiments to CineMind, a deployed film-discovery platform now live and in active iteration.',
    // ---- Detail-page fields (rendered by the WorkDetail screen via bySlug) ----
    tagline: 'June 2025 - Now',
    writeup: [
      "Most of the experiences that shaped me before Meta flexed an entrepreneurial muscle. Meta supplemented all of it with the thing I couldn't get anywhere else: 3 years of operating at scale in a large network of engineers and partners, fast execution with a high quality bar, and the flexibility to wear whatever hat the product needed. By 2025 I was ready to combine the two halves, and the next steps in my growth as an engineer (greater responsibility, broader ownership, decisions that define a product rather than execute one) didn't have clear paths within my current position.",
      "On top of this developments in LLMs and agents presented an opportunity I couldn't ignore. It was a wild west, full of new problems to solve and new products that couldn't have existed a year earlier, and it was fundamentally changing how software engineers work. I knew the technology was revolutionary enough to be worth a real risk. So I went all in: untethered from established companies that would reasonably take their time adopting it, forced to do every part myself, dedicated full-time to building real products on this new stack.",
      "That's what this year is. It ran from small focused experiments up to CineMind, a deployed film-discovery platform that's live and in active iteration right now. This page tells the story of the arc, the individual project pages hold the deep dives.",

      { heading: 'A year with a shape' },
      "The goal was simple: get myself in a position where I could build a true AI-native product at production quality. My early projects worked towards this in a repeating pattern: take a technology I had just learned and apply it to a real use case. They started small and grew into more complex products solving real problems, and I grew with each one. They serve as the foundation on which CineMind stands.",
      "The first experiments were deliberately small. A receipt-splitter chatbot was my first LLM workflow, and it's where 'anything that can be done with code must be done with code' first crystallized. Wizard Battle was a focused experiment in structured generation and in representing a convoluted environment in a way that natural-language-based models could operate in successfully. NerdBot raised the bar with an expertise agent using custom tools built on top of raw APIs.",
      "InterviewPro was the real product: a full, complex LLM-native system I genuinely deployed (and am currently using in my job search). Multi-agent research on a company and role, an adaptive mock interview over voice or text, and 6 evaluation passes judging your answers in parallel, each through its own narrow lens, all wrapped in a user-facing product with the guardrails and polish a public app demands.",
      "And CineMind is the payoff, my production-grade magnum opus. It's a film discovery platform built around describing what you're in the mood for, and it demanded every skill from the year at once: a real user problem, search across complementary semantic vector spaces, LLM evaluation at scale, an ingestion pipeline that narrowed roughly 1M candidate films down to the 120K worth representing well, a full frontend and backend, and a real deployment with a thought-through threat model behind it. It's live, it's stable, and it's in active iteration. The full breakdown is on its page, and it's the single best answer I have to the question of what I can do on my own.",

      { heading: 'The lessons that repeated' },
      "The most transferable thing I took from the year isn't any one system, it's a way of building with LLMs that held up across every project. The core of it: anything that can be done with code must be done with code. LLMs are slow, expensive, and probabilistic, so they only get to own the part of a problem that genuinely needs them, and most of the engineering exists to shrink that part down.",
      "A handful of rules sit underneath that, each one earned in a specific build. Shrink the model's job into language-friendly form: in Nerdbot rather than having the LLM directly understand platform IDs I turned them into enums based on their names and the model showed far greater accuracy. Put reasoning before the answer, because a justification written after a decision doesn't shape it, it just defends it. And be aggressive in running any calls that don't depend on each other in parallel to reduce latency.",
      "The other two run deeper than prompting. Decisions come from concrete evaluation, even in subjective spaces where you have to design the rubric and the test set yourself, because 'it looks better to me' is not evidence. And context management is the greatest differentiator between an engineer that can prompt and an engineer that truly knows how to squeeze every bit of value from the models they have.",
      "The year also turned AI-assisted development itself into a discipline I take seriously. I didn't just use agentic coding tools, I engineered around them: a custom memory system for my Claude Code setup with tiered context docs that load on demand, a running log that captures decisions while they're fresh, and human-gated review before any learning becomes a permanent convention. The tooling is a genuine multiplier, but only if you treat working with it as an engineering practice of its own.",

      { heading: 'Where it stands' },
      "Here's the ledger. Several deployed products, all personally funded and operated (CineMind's LLM generation across development and the full ingestion came to roughly $1,200, fronted by me because I believed the product was worth it). CineMind is live, and I'm actively iterating on it to get it to the state where I can start advertising it. I now have a unique depth of understanding of how LLMs work, born both from a need to minimize costs due to self-funding and because I built everything myself.",
      "I've accomplished what I set out to do and I feel I am in a great position to continue growing in a fast-paced environment of like-minded engineers working to solve real problems while playing to the strengths of a new, revolutionary technology."
    ],
  },
  {
    slug: 'instagram',
...
/**
 * The experience-only entries (independent-ai-product-development, facebook, mealme,
 * dcu) live solely in EXPERIENCES;
